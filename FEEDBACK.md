...
Written while building Turing Pool at ETHGlobal Lisbon 2026. We integrated AgentKit twice: off-chain (the official `@worldcoin/agentkit@0.2.0` SDK gating a quote API) and on-chain (reading `AgentBook.lookupHuman` from inside AMM contracts on Base). This file is the developer-feedback deliverable the World prize brief asks for.
...
3. **Canonical chain ambiguity (Base vs World Chain).** The docs say lookups resolve "against World Chain as the canonical deployment", but the registration relay docs reference Base, and `createAgentBookVerifier` pins World Chain unless you override `client`/`contractAddress`/`rpcUrl`. We needed Base (to compose with 1inch Aqua on the same chain) and used our own `readContract` with the Base deployment (`0xE1D1...61A4`). Please document: which deployments are kept in sync, and which is authoritative.
