...
Written while building Turing Pool at ETHGlobal Lisbon 2026. We integrated AgentKit twice: off-chain (the official `@worldcoin/agentkit@0.2.0` SDK gating a quote API) and on-chain (reading canonical World Chain AgentBook directly from SwapVM opcode 34). This file is the developer-feedback deliverable the World prize brief asks for.
...
3. **Same-chain execution is the cleanest contract integration.** AgentKit can sign on several EVM chains while `createAgentBookVerifier` resolves canonical identity on World Chain. Running our Aqua market on World Chain let opcode 34 read AgentBook synchronously without another trust boundary.
...
6. **Registration requires World App + (apparently) Orb-level verification; the required `groupId` isn't documented.** For local contract checks we registered agents through isolated fixture state. A sandbox registration path with test credentials would make it easier for teams to exercise the real World App flow before using canonical World Chain state.
